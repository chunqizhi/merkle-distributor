...
func TestRBalanceTree(t *testing.T) {
	...
	tree, err := NewResultRankTree(resultRanks)
	assert.Nil(t, err)
	...
	for idx, resultRank := range resultRanks {
		p := make(Elements, 0)
		for _, s := range proofs[idx] {
			p = append(p, common.HexToHash(s))
		}
		fmt.Println(resultRank)
		fmt.Println(p)
		fmt.Println()
		assert.True(t, ResultRankVerifyProof(resultRank.Account, resultRank.Amount, p, root))
	}
